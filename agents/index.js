...
const { runFollowUpPipeline } = require('./tasks/followup-pipeline');
const { runVoiceWatchdog } = require('./tasks/voice-watchdog');
...
async function main() {
  ...
  try {
    switch (task) {
      ...
      case 'followup-pipeline': {
        await runFollowUpPipeline();
        break;
      }

      case 'voice-watchdog': {
        await runVoiceWatchdog();
        break;
      }
      ...
      default:
        ...
        console.error('Available: daily-brief, outreach-pipeline, followup-pipeline, voice-watchdog, research, deep-research, outreach, campaign-analysis, test');
        process.exit(1);
    }
  } catch (err) {
